...
let dos_carte = ["/labo2/images/carte_dos.png"]
let len = liste_cartes.length

function affichageDos() {
    for (let i=0; i<len; i++) {
        document.querySelector("#_"+(i+1)).src = dos_carte
    }
}
...
window.addEventListener('load', affichageDos)
...
let btn_affichage = document.querySelector('#btn_affichage')
btn_affichage.addEventListener('click', affichageDeck)

let btn_brassage = document.querySelector('#btn_brassage')
btn_brassage.addEventListener('click', brassage)
